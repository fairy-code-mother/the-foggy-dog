...
(function () {
  ...
  boostPFSRangeWidths = [180, 250, 360, 540, 720, 900, 1080, 1296, 1512, 1728, 2048];
...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // Do we need to add a collection image?
  var currentPage = parseInt(Utils.stripHtml(Globals.queryParams.page));
  if (!currentPage || isNaN(currentPage)) {
    currentPage = 1;
  }
  var featureContentHtml = '';
  var itemIndexNum = itemIndex;

  if(itemIndexNum === 9) {
    if (boostPFSTemplate.collectionFeature !== '') {
      featureContentHtml = boostPFSTemplate.collectionFeature;
    }
  }
  ...
  if (featureContentHtml) {
    return featureContentHtml + itemHtml;
  }
...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    if (onSale) {
      ...
      html += '<span class="sale-price">' + Utils.formatMoney(data.price_min) + '</span>';
    } else {
      if (priceVaries) {
      ...
      if (data.tags) {
        for (var i = data.tags.length - 1; i >= 0; i--) {
          ...
          if (tag.indexOf("pfs:label") !== -1) {
            ...
            tagLabel += preTagLabel;
            break;
          } else if (tag.indexOf("new") !== -1 || tag.indexOf("New") !== -1) {
            var preTagLabel = boostPFSTemplate.tagLabelHtml.replace(/{{labelTag}}/g, "New Arrival");
            tagLabel += preTagLabel;
            break;
          } else if (tag.indexOf("best-seller") !== -1 || tag.indexOf("Best Seller") !== -1) {
            ...
            tagLabel += preTagLabel;
            break;
          } else if (tag.indexOf("oprah") !== -1 || tag.indexOf("Oprah") !== -1) {
...
ProductPaginationDefault.prototype.compileTemplate = function(totalProduct) {
  ...
  for (var iPage = 0; iPage < pageArr.length; iPage++) {
    ...
    } else if (iPage + 1 == pageArr.length) {
...
Filter.prototype.afterRender = function(data) {
  ...
  jQ('.boost-pfs-filter-total-product, .boost-product-count').html(totalProduct);
...
FilterApi.afterCallAsync = function (result, callbackFilterApi) {
  ...
  if (typeof window.Locksmith === 'object' && result.products.length > 0) {
    var URLs = [],
      ...
      counter = 0,
      URLsChunks = [],
      grantedAccess = new Array(result.products.length);
    //Create an array of products URLs
    for (var i = 0; i < result.products.length; i++) {
      URLs.push('/products/' + result.products[i].handle);
    }

    //Divide array into chuncks of 20, more than 20 cause an error in the request.
    for (var i = 0; i < URLs.length; i += chunk) {
      URLsChunks.push(URLs.slice(i, i + chunk));
    }

    URLsChunks.forEach(function (URLsChunk, index) {
      jQ.ajax({
        method: 'GET',
        ...
        success: function (response) {
          try {
            ...
            //Temp arrayForSavingProducts
            var accesibleProducts = [];
            for (var i = 0; i < URLsChunk.length; i++) {
              //Checks that the response exists
              if (typeof resources[URLsChunk[i]] !== 'undefined' && (!resources[URLsChunk[i]].hide_resource || resources[URLsChunk[i]].manual_lock)) {
                //Checks that the access of the product has been granted
                if (!resources[URLsChunk[i]].hide_resource) {
                  //If the product is accesible add it to the array of the products
                  grantedAccess.splice((chunk * index) + i, 1, result.products[(chunk * index) + i])
                }
              }
              counter++;
            }
            if (counter === result.products.length) {
              var removedProducts = [];
              grantedAccess.forEach(function (product) {
                if (typeof product === "object") {
                  removedProducts.push(product)
                }
              })
              result.products = removedProducts;
            }


            /* Build everything after getting filter data */
            if (index == URLsChunks.length - 1) callbackFilterApi(result);
          } catch (error) {
            console.error(error);
